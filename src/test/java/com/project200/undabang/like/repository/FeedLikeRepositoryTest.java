...
class FeedLikeRepositoryTest {
    ...
    @Nested
    @DisplayName("findByFeedAndMember 메소드는")
    class FindByFeedAndMember {

        @Test
        @DisplayName("피드와 회원이 주어지면 해당 좋아요를 반환한다")
        void findByFeedAndMember_success() {
            // given
            Member member = createAndSaveMember();
            FeedType feedType = createAndSaveFeedType();
            Feed feed = createAndSaveFeed(member, feedType);
            FeedLike feedLike = createAndSaveFeedLike(feed, member);

            em.flush();
            em.clear();

            // when
            Optional<FeedLike> foundLike = feedLikeRepository.findByFeedAndMember(feed, member);
        ...
        void findByFeedAndMember_isEmpty() {
            // given
            Member member = createAndSaveMember();
            FeedType feedType = createAndSaveFeedType();
            Feed feed = createAndSaveFeed(member, feedType);
    ...
    private FeedLike createAndSaveFeedLike(Feed feed, Member member) {
