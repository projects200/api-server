...
class SimpleTimerRepositoryTest {
    ...
    @Nested
    @DisplayName("countDistinctByMemberAndSimpleTimerDeletedAtNull 메소드는")
    class Describe_countDistinctByMemberAndSimpleTimerDeletedAtNull {

        @Test
        @DisplayName("특정 회원이 가진 삭제되지 않은 심플 타이머의 개수를 정확히 반환한다")
        void it_returns_the_correct_count_of_non_deleted_timers() {
            // given
            Member member1 = createAndSaveMember("testMember1");
            Member member2 = createAndSaveMember("testMember2");

            // member1의 삭제되지 않은 타이머
            createAndSaveSimpleTimer(member1, 30);
            createAndSaveSimpleTimer(member1, 40);

            // member1의 삭제된 타이머
            SimpleTimer deletedTimer = createAndSaveSimpleTimer(member1, 50);
            deletedTimer.deleteSimpleTimer();
            em.persist(deletedTimer);

            // member2의 타이머
            createAndSaveSimpleTimer(member2, 300);

            flushAndClear();

            // when
            int count = simpleTimerRepository.countDistinctByMemberAndSimpleTimerDeletedAtNull(member1);
        ...
        @Test
        @DisplayName("심플 타이머가 없는 회원을 조회하면 0을 반환한다")
        void it_returns_zero_when_no_timers_exist() {
            // given
        ...
        @Test
        @DisplayName("모든 심플 타이머가 삭제된 회원을 조회하면 0을 반환한다")
        void it_returns_zero_when_all_timers_are_deleted() {
            // given
            Member member = createAndSaveMember("testMember");
            SimpleTimer timer1 = createAndSaveSimpleTimer(member, 30);
            SimpleTimer timer2 = createAndSaveSimpleTimer(member, 40);

            timer1.deleteSimpleTimer();
            timer2.deleteSimpleTimer();
            em.persist(timer1);
            em.persist(timer2);
