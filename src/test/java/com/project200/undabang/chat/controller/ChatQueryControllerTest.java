...
class ChatQueryControllerTest extends AbstractRestDocSupport {
    ...
    class GetNewMessages {
        ...
        void getNewMessages_Success() throws Exception {
            ...
            GetNewChatResponse mockResponse = GetNewChatResponse.of(newMessages, true, false); // 상대방 활성, 차단 안됨
            ...
            mockMvc.perform(get("/api/v1/chat-rooms/{chatroomId}/messages/new", chatroomId)
                    ...
                    .andExpect(jsonPath("$.data.opponentActive").value(true))
                    .andExpect(jsonPath("$.data.blockActive").value(false)) // [수정] 검증 추가
                    .andDo(document.document(
                            requestHeaders(HEADER_ACCESS_TOKEN),
                            pathParameters(
                            ...
                            responseFields(commonResponseFields(
                                    ...
                                    fieldWithPath("data.blockActive").type(JsonFieldType.BOOLEAN).description("상호간의 차단 여부를 나타냅니다. (내가 상대를 차단했거나, 상대가 나를 차단한지의 여부)") // [수정] 문서 추가
                            ))
                    ));
        }
        ...
        void getNewMessages_Success_Empty() throws Exception {
            // given
            UUID memberId = UUID.randomUUID();
            GetNewChatResponse mockResponse = GetNewChatResponse.of(Collections.emptyList(), true, true); // 메시지 없음, 상대방 활성, 차단됨
            ...
            mockMvc.perform(get("/api/v1/chat-rooms/{chatroomId}/messages/new", chatroomId)
                    ...
                    .andExpect(jsonPath("$.data.opponentActive").value(true))
                    .andExpect(jsonPath("$.data.blockActive").value(true)); // [수정] 검증 추가
        }
