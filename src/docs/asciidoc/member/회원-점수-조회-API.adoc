...
- 회원의 점수를 조회하는 API입니다.
- 이 API는 회원의 식별자, 점수, 정책상 최대 점수와 최소 점수를 반환합니다.
- 회원의 운동 점수는 정책상 최소 점수와 최대 점수 사이에 위치합니다.

=== 개발 이력

- Sprint 8 (2025-08-04): 응답 구조 변경이 완료되었습니다.
* 기존 응답 데이터에 정책 데이터인 최대 점수와 최소 점수가 포함 되었습니다.
