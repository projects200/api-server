...
public class GlobalExceptionHandler {
    ...
    /**
     * 필수 요청 헤더가 누락되었을 때 발생하는 예외를 처리합니다.
     *
     * <p>이 메서드는 {@link MissingRequestHeaderException} 타입의 예외를 처리하며,
     * 누락된 헤더 정보를 포함한 메시지를 클라이언트에 전달합니다.</p>
     *
     * @param ex 처리할 {@link MissingRequestHeaderException} 예외
     * @return 누락된 헤더 정보를 포함한 오류 응답
     */
    @ExceptionHandler(MissingRequestHeaderException.class)
    @ResponseStatus(HttpStatus.BAD_REQUEST)
    protected ResponseEntity<CommonResponse<Map<String, String>>> handleMissingRequestHeaderException(
            MissingRequestHeaderException ex) {
        log.warn("필수 헤더 누락: {}", ex.getMessage());
        ErrorCode errorCode = ErrorCode.INVALID_INPUT_VALUE;

        Map<String, String> errors = new HashMap<>();
        errors.put(ex.getHeaderName(), "필수 헤더가 누락되었습니다");

        String customMessage = "필수 헤더를 포함해주세요.";
